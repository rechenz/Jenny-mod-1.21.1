...
extends MessageFormat {
    ...
    public final void applyPattern(String pattern) {
        ...
        char[] c10 = pattern.toCharArray();
        int fmtCount = 0;
        block4: while (pos.getIndex() < pattern.length()) {
            switch (c10[pos.getIndex()]) {
                case '\'': {
                ...
                case '{': {
                    ...
                    if (c10[pos.getIndex()] == ',' && (format = this.getFormat(formatDescription = this.parseFormatDescription(pattern, this.next(pos)))) == null) {
                    ...
                    if (c10[pos.getIndex()] == '}') break;
                    throw new IllegalArgumentException("Unreadable format element at position " + start);
                }
            }
            stripCustom.append(c10[pos.getIndex()]);
            this.next(pos);
        }
    ...
    private String insertFormats(String pattern, ArrayList<String> customPatterns) {
        ...
        block5: while (pos.getIndex() < pattern.length()) {
            char c10 = pattern.charAt(pos.getIndex());
            switch (c10) {
                case '\'': {
    ...
    private StringBuilder appendQuotedString(String pattern, ParsePosition pos, StringBuilder appendTo) {
        ...
        for (int i2 = pos.getIndex(); i2 < pattern.length(); ++i2) {
            if (c10[pos.getIndex()] == '\'') {
                this.next(pos);
                return appendTo == null ? null : appendTo.append(c10, lastHold, pos.getIndex() - lastHold);
            }
            this.next(pos);
        }
