...
public class TypeUtils {
    ...
    private static Map<TypeVariable<?>, Type> getTypeArguments(ParameterizedType parameterizedType, Class<?> toClass, Map<TypeVariable<?>, Type> subtypeVarAssigns) {
        ...
        for (int i2 = 0; i2 < typeParams.length; ++i2) {
            ...
            typeVarAssigns.put(typeParams[i2], typeVarAssigns.getOrDefault(typeArg, typeArg));
        }
    ...
    public static String toLongString(TypeVariable<?> var) {
        ...
        block5: {
            ...
            Object d10 = var.getGenericDeclaration();
            if (d10 instanceof Class) {
                Class<?> c10 = (Class<?>)d10;
                while (true) {
                    if (c10.getEnclosingClass() == null) {
                        buf.insert(0, c10.getName());
                        break block5;
                    }
                    buf.insert(0, c10.getSimpleName()).insert(0, '.');
                    c10 = c10.getEnclosingClass();
                }
            }
            if (d10 instanceof Type) {
                buf.append(TypeUtils.toString((Type)d10));
            } else {
                buf.append(d10);
            }
        }
    ...
    private static void appendRecursiveTypes(StringBuilder builder, int[] recursiveTypeIndexes, Type[] argumentTypes) {
        for (int i2 = 0; i2 < recursiveTypeIndexes.length; ++i2) {
            TypeUtils.appendAllTo(builder.append('<'), ", ", argumentTypes[i2].toString()).append('>');
        }
    ...
    private static <T> StringBuilder appendAllTo(StringBuilder builder, String sep, T ... types) {
        ...
        if (types.length > 0) {
            ...
            for (int i2 = 1; i2 < types.length; ++i2) {
                builder.append(sep).append(TypeUtils.toString(types[i2]));
            }
        }
        return builder;
    }
