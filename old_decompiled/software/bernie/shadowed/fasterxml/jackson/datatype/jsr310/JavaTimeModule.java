...
import software.bernie.shadowed.fasterxml.jackson.datatype.jsr310.PackageVersion;
        ...
        context.addValueInstantiators(new ValueInstantiators.Base(){
            ...
            public ValueInstantiator findValueInstantiator(DeserializationConfig config, BeanDescription beanDesc, ValueInstantiator defaultInstantiator) {
                ...
                if (ZoneId.class.isAssignableFrom(raw) && defaultInstantiator instanceof StdValueInstantiator) {
                    ...
                    AnnotatedClass ac2 = raw == ZoneId.class ? beanDesc.getClassInfo() : AnnotatedClassResolver.resolve(config, config.constructType(ZoneId.class), config);
                    if (!inst.canCreateFromString() && (factory = JavaTimeModule.this._findFactory(ac2, "of", String.class)) != null) {
    ...
    protected AnnotatedMethod _findFactory(AnnotatedClass cls, String name, Class<?> ... argTypes) {
        ...
        for (AnnotatedMethod method : cls.getFactoryMethods()) {
            ...
            for (int i2 = 0; i2 < argCount; ++i2) {
                Class<?> argType = method.getParameter(i2).getRawType();
                if (argType.isAssignableFrom(argTypes[i2])) continue;
            }
            return method;
        }
        return null;
