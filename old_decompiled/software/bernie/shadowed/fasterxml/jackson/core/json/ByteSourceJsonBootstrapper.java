...
import software.bernie.shadowed.fasterxml.jackson.core.json.UTF8StreamJsonParser;
...
public final class ByteSourceJsonBootstrapper {
    ...
    public static int skipUTF8BOM(DataInput input) throws IOException {
        int b10 = input.readUnsignedByte();
        if (b10 != 239) {
            return b10;
        }
        b10 = input.readUnsignedByte();
        if (b10 != 187) {
            throw new IOException("Unexpected byte 0x" + Integer.toHexString(b10) + " following 0xEF; should get 0xBB as part of UTF-8 BOM");
        }
        b10 = input.readUnsignedByte();
        if (b10 != 191) {
            throw new IOException("Unexpected byte 0x" + Integer.toHexString(b10) + " following 0xEF 0xBB; should get 0xBF as part of UTF-8 BOM");
        }
    ...
    public static MatchStrength hasJSONFormat(InputAccessor acc) throws IOException {
        ...
        if ((ch2 = ByteSourceJsonBootstrapper.skipSpace(acc, b10)) < 0) {
            return MatchStrength.INCONCLUSIVE;
        }
        if (ch2 == 123) {
            ch2 = ByteSourceJsonBootstrapper.skipSpace(acc);
            if (ch2 < 0) {
                return MatchStrength.INCONCLUSIVE;
            }
            if (ch2 == 34 || ch2 == 125) {
        ...
        if (ch2 == 45) {
            ...
            return ch2 <= 57 && ch2 >= 48 ? strength : MatchStrength.NO_MATCH;
        }
        if (ch2 == 110) {
    ...
    private static MatchStrength tryMatch(InputAccessor acc, String matchStr, MatchStrength fullMatchStrength) throws IOException {
        ...
        for (int i2 = 0; i2 < len; ++i2) {
            ...
            if (acc.nextByte() == matchStr.charAt(i2)) continue;
            return MatchStrength.NO_MATCH;
        }
    ...
    private static int skipSpace(InputAccessor acc, byte b10) throws IOException {
        int ch2;
        while ((ch2 = b10 & 0xFF) == 32 || ch2 == 13 || ch2 == 10 || ch2 == 9) {
            if (!acc.hasMoreBytes()) {
                return -1;
            }
            b10 = acc.nextByte();
