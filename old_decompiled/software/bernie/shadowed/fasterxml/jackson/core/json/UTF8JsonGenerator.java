...
extends JsonGeneratorImpl {
    ...
    public final void writeRaw(char[] cbuf, int offset, int len) throws IOException {
        ...
        block0: while (offset < len) {
            ...
            if (ch2 < '\u0800') {
                ...
                this._outputBuffer[this._outputTail++] = (byte)(0x80 | ch2 & 0x3F);
                continue;
            }
    ...
    public void writeRaw(char ch2) throws IOException {
        ...
        if (ch2 <= '\u007f') {
            ...
            bbuf[this._outputTail++] = (byte)(0x80 | ch2 & 0x3F);
        } else {
            this._outputRawMultiByteChar(ch2, null, 0, 0);
        }
    }
    ...
    protected final void _verifyValueWrite(String typeMsg) throws IOException {
        ...
        this._outputBuffer[this._outputTail++] = b10;
    }
    ...
    private final void _writeStringSegment(char[] cbuf, int offset, int len) throws IOException {
        ...
        while (offset < len && (ch2 = cbuf[offset]) <= '\u007f' && escCodes[ch2] == 0) {
    ...
    private final void _writeStringSegment(String text, int offset, int len) throws IOException {
        ...
        byte[] outputBuffer = this._outputBuffer;
        int[] escCodes = this._outputEscapes;
        while (offset < len && (ch2 = text.charAt(offset)) <= '\u007f' && escCodes[ch2] == 0) {
            outputBuffer[outputPtr++] = (byte)ch2;
            ++offset;
        }
        this._outputTail = outputPtr;
    ...
    private final void _writeStringSegment2(char[] cbuf, int offset, int end) throws IOException {
        ...
        while (offset < end) {
            ...
            if ((ch2 = cbuf[offset++]) <= '\u007f') {
                ...
                outputPtr = this._writeGenericEscape(ch2, outputPtr);
                continue;
            }
            if (ch2 <= '\u07ff') {
                outputBuffer[outputPtr++] = (byte)(0xC0 | ch2 >> 6);
                outputBuffer[outputPtr++] = (byte)(0x80 | ch2 & 0x3F);
                continue;
            }
            outputPtr = this._outputMultiByteChar(ch2, outputPtr);
        }
        this._outputTail = outputPtr;
    }
        ...
        while (offset < end) {
            ...
            if ((ch2 = cbuf[offset++]) <= 127) {
                ...
                if (escape == -2) {
                    ...
                    if (esc == null) {
                        this._reportError("Invalid custom escape definitions; custom escape not found for character code 0x" + Integer.toHexString(ch2) + ", although was supposed to have one");
                    }
    ...
    private final void _writeUTF8Segment(byte[] utf8, int offset, int len) throws IOException, JsonGenerationException {
        ...
        while (ptr < end) {
            byte ch2;
            if ((ch2 = utf8[ptr++]) < 0 || escCodes[ch2] == 0) continue;
    ...
    private final void _writeUTF8Segment2(byte[] utf8, int offset, int len) throws IOException, JsonGenerationException {
        ...
        while (offset < len) {
            byte b10;
            byte ch2;
            if ((ch2 = (b10 = utf8[offset++])) < 0 || escCodes[ch2] == 0) {
                outputBuffer[outputPtr++] = b10;
                continue;
            }
            int escape = escCodes[ch2];
            if (escape > 0) {
    ...
    private final int _outputRawMultiByteChar(int ch2, char[] cbuf, int inputOffset, int inputEnd) throws IOException {
        if (ch2 >= 55296 && ch2 <= 57343) {
            if (inputOffset >= inputEnd || cbuf == null) {
                this._reportError(String.format("Split surrogate on writeRaw() input (last character): first character 0x%4x", ch2));
            }
            this._outputSurrogates(ch2, cbuf[inputOffset]);
            return inputOffset + 1;
        }
        byte[] bbuf = this._outputBuffer;
        bbuf[this._outputTail++] = (byte)(0xE0 | ch2 >> 12);
        bbuf[this._outputTail++] = (byte)(0x80 | ch2 >> 6 & 0x3F);
        bbuf[this._outputTail++] = (byte)(0x80 | ch2 & 0x3F);
        return inputOffset;
    }

    protected final void _outputSurrogates(int surr1, int surr2) throws IOException {
        int c10 = this._decodeSurrogate(surr1, surr2);
        if (this._outputTail + 4 > this._outputEnd) {
            this._flushBuffer();
        }
        byte[] bbuf = this._outputBuffer;
        bbuf[this._outputTail++] = (byte)(0xF0 | c10 >> 18);
        bbuf[this._outputTail++] = (byte)(0x80 | c10 >> 12 & 0x3F);
        bbuf[this._outputTail++] = (byte)(0x80 | c10 >> 6 & 0x3F);
        bbuf[this._outputTail++] = (byte)(0x80 | c10 & 0x3F);
    }
    ...
    private final int _outputMultiByteChar(int ch2, int outputPtr) throws IOException {
        ...
        if (ch2 >= 55296 && ch2 <= 57343) {
            bbuf[outputPtr++] = 92;
            bbuf[outputPtr++] = 117;
            bbuf[outputPtr++] = HEX_CHARS[ch2 >> 12 & 0xF];
            bbuf[outputPtr++] = HEX_CHARS[ch2 >> 8 & 0xF];
            bbuf[outputPtr++] = HEX_CHARS[ch2 >> 4 & 0xF];
            bbuf[outputPtr++] = HEX_CHARS[ch2 & 0xF];
        } else {
            bbuf[outputPtr++] = (byte)(0xE0 | ch2 >> 12);
            bbuf[outputPtr++] = (byte)(0x80 | ch2 >> 6 & 0x3F);
            bbuf[outputPtr++] = (byte)(0x80 | ch2 & 0x3F);
        }
        return outputPtr;
    }
