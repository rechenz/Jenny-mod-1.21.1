...
public final class TextBuffer {
    ...
    public void append(char c10) {
        ...
        curr[this._currentSize++] = c10;
    }

    public void append(char[] c10, int start, int len) {
        int amount;
        if (this._inputStart >= 0) {
            this.unshare(len);
        }
        ...
        if (max >= len) {
            System.arraycopy(c10, start, curr, this._currentSize, len);
            this._currentSize += len;
            return;
        }
        if (max > 0) {
            System.arraycopy(c10, start, curr, this._currentSize, max);
            start += max;
            len -= max;
        }
        do {
            ...
            System.arraycopy(c10, start, this._currentSegment, 0, amount);
