...
implements Serializable {
    ...
    protected StringBuilder _appendSourceDesc(StringBuilder sb) {
        ...
        if (srcRef instanceof CharSequence) {
            ...
            len = cs2.length();
            len -= this._append(sb, cs2.subSequence(0, Math.min(len, 500)).toString());
        } else if (srcRef instanceof char[]) {
            char[] ch2 = (char[])srcRef;
            len = ch2.length;
            len -= this._append(sb, new String(ch2, 0, Math.min(len, 500)));
        } else if (srcRef instanceof byte[]) {
            byte[] b10 = (byte[])srcRef;
            int maxLen = Math.min(b10.length, 500);
            this._append(sb, new String(b10, 0, maxLen, Charset.forName("UTF-8")));
            len = b10.length - maxLen;
            charStr = " bytes";
        } else {
            len = 0;
        }
